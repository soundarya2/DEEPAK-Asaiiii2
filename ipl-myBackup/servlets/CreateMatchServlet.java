...
public class CreateMatchServlet extends SlingAllMethodsServlet {
	...
	protected void doPost(final SlingHttpServletRequest req, final SlingHttpServletResponse resp) throws ServletException, IOException{
		...
		try {
			Boolean createMatch = createMatchObject.createMatch(teamOne, teamTwo, rules, overs, tossWinner, chooseTo, teamOnePlayingSquadList, teamTwoPlayingSquadList,resolver);
			System.out.println(createMatch);
			resp.getWriter().print(createMatch);
