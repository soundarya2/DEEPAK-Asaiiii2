...
public class MatchOperationsImpl implements MatchOperations {
	...
	public Node getCurrentMatch(ResourceResolver resolver) throws RepositoryException {
		Resource matchResource = resolver.getResource("/content/ipl-scorer/schema/match_schema");
		Node matchSchemaNode = matchResource.adaptTo(Node.class);
		String[] currentMatch = matchSchemaNode.getProperty("current_match_node").getString().split("/");
		
		return matchSchemaNode.getNode(currentMatch[currentMatch.length - 1]);
	...
	public void updateScore(ResourceResolver resolver, String currentMatchPath, String currentInnings, String currentBowlerPath,
			...
			boolean isWicket, boolean isBallValid, boolean isPowerPlay,Integer firstInningsTotal, Integer firstInningsWickets,
			Integer secondInningsTotal, Integer secondInningsWickets, Integer extras, String[] thisOver,Integer runsToBeUpdated, Integer overs) throws UnsupportedRepositoryOperationException, RepositoryException {
		...
		//power play
		matchNode.setProperty("is_power_play", isPowerPlay);
		...
		if(currentBall == 6 && currentOver+1 == overs) {
			
			if(matchNode.getProperty("current_innings").getString().equals("second")) {
				matchNode.setProperty("is_match_over", true);
			}
			matchNode.setProperty("current_batsman", "-");
			matchNode.setProperty("current_bowler", "-");
			matchNode.setProperty("current_batsman_name", "-");
			matchNode.setProperty("current_bowler_name", "-");
			matchNode.setProperty("current_over", 0);
			matchNode.setProperty("current_ball", 0);
			matchNode.setProperty("total_runs", 0);
			matchNode.setProperty("extras" , 0);
			matchNode.setProperty("is_power_play", false);
			matchNode.setProperty("current_over_actual_ball", 0);
			matchNode.setProperty("current_innings", "second");
