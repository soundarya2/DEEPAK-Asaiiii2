...
public class CreateMatchImpl implements CreateMatch {
	...
	@Override
	public Boolean createMatch(String teamOne, String teamTwo, String rules, String overs, String tossWinner, String chooseTo,
			List<String> teamOnePlayingSquadList, List<String> teamTwoPlayingSquadList,ResourceResolver resolver) throws RepositoryException {
        ...
        if(!isCurrentMatchOver(matchSchema, resolver)) {
        	
        	Node matchNode = matchSchema.addNode(matchName);
        	matchSchema.setProperty("current_match_node",matchNode.getPath());
        	
        	//match essentials
        	matchNode.setProperty("is_current_match", true);
        	int totalOvers = Integer.parseInt(overs);
        	matchNode.setProperty("total_overs", totalOvers);
	...
	public Boolean isCurrentMatchOver(Node matchSchemaNode , ResourceResolver resolver) throws RepositoryException {
		if(matchSchemaNode.hasProperty("current_match_node")) {
			Resource currentMatchResource = resolver.getResource(matchSchemaNode.getProperty("current_match_node").getString());
			Node currentMatchNode = currentMatchResource.adaptTo(Node.class);
			return currentMatchNode.getProperty("is_match_over").getBoolean();
		}
		return false;
	}
